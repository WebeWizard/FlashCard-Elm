...
import Element exposing (Element, alignRight, rgb, el, fill, htmlAttribute, link, paddingXY, px, rgb255, row, spacing, text, width)
...
cardBox toMsg edit cardInfo =
    ...
    in
        let
            cardButton = cardButtonBuilder toMsg displayInfo
            cardInput = cardInputBuilder toMsg displayInfo
        in
...
cardButtonBuilder toMsg cardInfo mode  =
    button
        ...
        , label = text (
            case mode of
                Pos -> String.fromInt cardInfo.pos
...
cardInputBuilder toMsg displayInfo mode =
    Input.text
        ...
        , text =
            (case mode of
                Pos -> String.fromInt displayInfo.pos
                Question -> displayInfo.question
                Answer -> displayInfo.answer
                Uploading -> "uploading.."
            )
        , placeholder = 
            let
        ...
        , label = labelHidden (case mode of
                Pos -> "Position"
                Question -> "Question"
                Answer -> "Answer"
                Uploading -> "uploading.."
